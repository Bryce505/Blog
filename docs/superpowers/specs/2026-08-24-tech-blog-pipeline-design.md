...
| 检验方式 | 结果 |
|---|---|
| 误报：模拟合理重组（打乱段落 + 加导读句 + 删 10% 段落） | 33 组中 2 组报警，且两组均为真阳性（一组丢 47% 文献引用，一组丢 ICH Q5E/Q6B/Ph.Eur.） |
| 漏报：注入源文中不存在的数值（小数/大整数/百分比/带单位值） | **131 次注入，0 次漏过** |
| 法规条款零容忍 | 12 个含法规条款的组，删除条款后 **0 次漏拦** |

误报测试用随机删段落模拟，比真实情况更严苛（真实提示词要求保留含引用的句子），实际误报率应低于此。
