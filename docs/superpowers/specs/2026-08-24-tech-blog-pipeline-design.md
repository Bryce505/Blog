...
| 书籍整章转录 | 单篇超 3 万字符即排除，作者原创长文走豁免名单放行 | 用户决策。实测 15 篇是整书/整章转录（整本《Capillary Electrophoresis Methods》142 万字符等），公开重组发布版权风险高于 Clippings |
| 分组粒度 | 按每篇笔记自身最深的标签层级（≤三级）分桶，碎组不向上归并 | 早期版本把碎组回退到二级合并，实测让「05仪器与分析技术/质谱」一组吃进 12 篇横跨五个子题的笔记，文章成了大杂烩。改为不归并 + `MIN_GROUP=2` 后：38 组覆盖 209 篇可发布笔记中的 150 篇，全部同子题 |
| AI 改写力度 | 方案 C：结构重组成一篇文章，但含数字/引文/图片引用的句子原样保留 + 机械校验兜底 | 兼顾可读性与事实安全 |
| LLM | DeepSeek `deepseek-v4-pro`（1M 上下文，384K 最大输出） | 技术内容改写对保真度要求高。模型 ID 以 `GET /models` 为准 —— `deepseek-chat`/`deepseek-reasoner` 已退出清单。想省钱可换 `deepseek-v4-flash`，机械校验器会兜底 |
...
- 体量分布：>20 万字符 5 篇（整书导入）、5 万–20 万 8 篇（长章节）、1 万–5 万 44 篇、2 千–1 万 90 篇、<2 千 77 篇。**最大的 13 篇占全部内容的 76%**
- 三级标签分组（**实现后实测，取代早期探索的虚高估算**）：**209 篇可发布 → 32 个合格组，覆盖 183 篇**（体量剔除 15 篇书籍转录，豁免名单放行 7 篇原创长文）

  > 为什么用豁免名单而不是自动判别书籍来源：实测数据里没有可靠信号 —— `book` 字段只填了 4/22，文件名模式要凑 `Chapter`/`作者_年份`/`Guidebook`/`course`/`Practitioners` 一串关键词才盖得全，新增一本书就漏。纯按体量切又会误伤作者自己写的长文（`重组尿酸氧化酶活性分析方法开发.md` 10.5 万字符、`QC检测流程梳理.md` 6.4 万字符等 7 篇，恰是技术博客最想要的内容）。只有二十来篇要分类，写分类器不如列名单：默认排除是版权问题的安全方向，原创的显式放行。新写了超 3 万字符的原创长文，往 `config.SIZE_EXEMPT_NOTES` 加一行路径即可。
...
2. 按标签建组（`02分子表征/PTM/糖基化`）。一篇笔记常有多个标签，归属规则：取其 frontmatter `tags` 数组中**出现顺序最靠前的、层级数 ≥3 的**标签；若无三级标签则取最靠前的二级标签。再按该标签的**前三级**分桶 —— 四级以下折到三级（再细每组只剩一篇），二级标签的笔记就按二级分桶（它本来就没有更细的粒度）。碎组不向上归并。一篇笔记只属于一组，避免同一内容在多篇文章里重复出现
...
- 端点 `https://api.deepseek.com/chat/completions`（OpenAI 兼容），model `deepseek-v4-pro`
- `max_tokens` 给到 65536：输出被截断会触发「正文过短」校验，白烧一次 token 还得重跑
...
| 法规条款完整性 | 源文中的 `ICH Qx`、`21 CFR x`、`USP <x>`、`Ph. Eur. x` **必须全部保留，零容忍** | AI 丢失或改错合规依据 |
| 文献引用完整性 | DOI 丢失不得超过 30%，且绝对下限允许丢 1 条 | AI 把整段参考内容丢了 |
| 长度合理性 | 输出正文不短于源文总字数的 40% | AI 大段丢失内容 |

法规与文献分开处理，是因为严重程度不同：法规条款号是规范性依据，丢一个就是合规主张变了；文献 DOI 是参考性的，结构重组合并重复论述时被一并折叠掉属正常，要求 100% 保留等于禁止重组。比例阈值必须配绝对下限，否则小分母失真（只有 3 条 DOI 时丢 1 条就是 33%）。

数字 token 抽取规则 —— **只检查承载数据的数字**，避免误杀 AI 写的导读句：

只有满足以下任一条件的数字才纳入比对：
- 带单位或百分号（`50 mM`、`4.6 mm`、`95%`、`25 °C`）—— 单位必须来自**真实单位白名单**，不能用「任意 1-6 个字母」：后者会把 `1The`、`12Figure`、`15piston` 这类「数字紧跟单词」当成数值，实测 33 个真实组里误报 14 组。白名单刻意不收裸 `A`（安培）和裸 `l`（升），它们会把 HPLC 的「流动相 A」匹配成 `36A`
- 含小数点（`0.45`、`7.4`）
- 三位及以上整数（`1200`、`280`）

DOI 正则用否定字符类而非 `\S+`：`\S+` 会把 markdown 链接语法、右括号、中文全角括号一起吞进来，同一个 DOI 因上下文不同被提取成不同字符串，触发假的「丢失引用」。
...
**`_review/` 里的组必须跳过。** 校验失败不写 `published.json`，若不跳过，`pick_next` 每次都会重新挑中它，队列被永久堵死、后面的文章一篇也发不出去。用文件系统当状态：`_review/<slug>.md` 存在即跳过，人工把文件移走或删掉，该组自动重新入列，不另设账本。

**实测效果**（33 个真实标签组）：

| 检验方式 | 结果 |
|---|---|
| 误报：按真实提示词约束模拟合理重组（保留含图片/法规/引用的段落，其余打乱并删 10%，加导读句） | **32 组 0 误报** |
| 漏报：注入源文中不存在的数值（小数/大整数/百分比/带单位值） | **128 次注入，0 次漏过** |
| 原样输入 | **32/32 组通过** |
| 法规条款零容忍 | 12 个含法规条款的组，删除条款后 **0 次漏拦** |

**比对前必须整体 URL 解码。** Obsidian 粘贴的图片名带空格，正文里是 `Pasted%20image%2020240528.png`。不解码有两处会错：图片 basename 对不上（每篇含空格图名的文章都误判丢图），且 `%2020240528` 会被数字校验读成 `2020240528`，凭空多出一个源文没有的「数据」。这个缺陷是端到端测试发现的，单元测试测不到。
...
- **流水线内（Python）**：双链 `[[目标]]` 解析 —— 目标笔记已发布则转**文章级**站内链接 `[显示名](/posts/<slug>)`，未发布则退化为纯文本（保留文字，去掉方括号），**不留死链**
  ...
  **双链与图片改写都必须跳过代码块**（围栏 ``` 与行内反引号）。Python 的嵌套列表 `[['及格', '不及格']]` 会被双链正则匹配，改写后代码直接损坏——实测全库 10 处。Obsidian 自身在代码块内也不渲染双链，跳过才是正确语义。
