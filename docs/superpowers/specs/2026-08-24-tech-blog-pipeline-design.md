...
| 分组粒度 | 三级标签为主分组单位；不足 3 篇的碎组回退到二级标签合并 | 实测：三级标签 109 组中 52 组落在 3~30 篇，无超大组 |
...
- 三级标签分组：109 组，其中 **52 组落在 3~30 篇**（合格），0 组超过 30 篇，57 组不足 3 篇（需回退合并）
...
3. 组内笔记数 3~30 为合格组；不足 3 篇的，回退到其二级标签与同级碎组合并；合并后仍不足 3 篇的挂起不发
