...
| **工具与效率** | 每次运行 | 把 RoutineRun 的工作笔记接成栏目，同样默认落草稿位等人工放行 |
...
**文件本身按发布日期新→旧排列，同一天内后写入的排更前**——在手机或网页里
直接打开 `published.json` 确认发表状态，最新一条就在文件最上面，不用拉到
文件末尾找。这层排序只在落盘前统一处理（`save_published()`），不管上面
的对账逻辑内部怎么增删记录，写到磁盘上的顺序始终正确。

#### 字段说明

每条记录用 slug 做 key，value 里这些字段：
...
| `title` | 文章标题 | 否——账本自动补建（见上）时若 frontmatter 没写 `title` 就没有 |
| `seed` | 引子通道用的源笔记路径（与 `notes[0]` 相同），供自动选材去重跳过用过的笔记 | 否——`auto` 通道多篇合并不写 |
| `draft` | 值恒为 `true` | 否——只有还压在草稿位、没正式上线才出现这个键；**正式发布的记录完全没有这个键，不是写 `false`** |
| `source_hash` | 源笔记内容的 sha256，判断源笔记有没有被改过；只有走 `record_published()` 的正常发布流程才写 | 否——账本自动补建的记录没有（补建时拿不到 vault，算不出哈希，上面说的「当作已发布、不重发」就是靠这个字段缺失判断的） |
| `noteTitles` | 与 `notes` 一一对应的笔记标题，供双链解析（笔记间的双链转成文章间的链接） | 否——同上，只有正常发布流程才写 |

> **这是当前状态的快照，不是变更历史。** `reconcile()` 每次运行都会拿
> `src/content/posts/` 的实际文件重新核对，字段随时可能被覆盖——比如人
> 删掉某篇文章 `draft: true` 那一行放行，这条记录的 `draft` 键就会消失，
> 不会留下"曾经是草稿"的痕迹。想看某条记录什么时候变成什么样，得翻这个
> 文件的 git 历史，不能只看当前内容。
...
`sync-reports` 改动过、或者想确认它还活着时，手机 GitHub App 按这个顺序点：

1. **Actions → `sync-reports` → 运行工作流**，分支 `master`。
   **安卓已知毛病**：一个 workflow 从没被手动跑过时这个按钮不出现（同
   「[手动触发一次发布](#手动触发一次发布)」那条）。绕法是用手机浏览器开
   Actions 页切「桌面版网站」，或者等周五的定时自己跑
2. **看日志该打印「周报没有更新」**。上游没出新一期时这就是正确结果 ——
   它证明「拷贝 → diff 判定」这条链路是通的，而且幂等
3. **看 Actions 列表里紧跟着有没有一条 `deploy`**。这条自动出现，才说明
   `deploy.yml` 的 `workflow_run` 列表里确实有 `sync-reports`；不出现的话
   同步的周报会永远躺在仓库里不上线
4. **上站看**：`https://bryce505.github.io/Blog/reports`，卡片数量与顶部那期
   的覆盖期对得上，随便点开一份确认渲染正常

想连「镜像语义」一起验掉，不用等新周报，两个方向各造一次，全程手机可完成：

| 造的状态 | 手机操作 | 跑完 `sync-reports` 应该看到 |
|---|---|---|
| 本地少一份（上游有） | 删掉 `public/reports/<年>/<某期>.html` 并提交 | 文件被拷回来，多一条「同步文献周报：…」提交 |
| 本地多一份（上游没有） | 新建一个 `public/reports/<年>/<假期号>.html` 提交 | 这份被清掉——上游撤下的旧期不会在站上留幽灵页面 |
...
### 定时任务怎么开关：生成候选 vs 直接发布

这是两层独立的开关，容易被当成一件事：

| 开关哪个 | 控制什么 | 现状 |
|---|---|---|
| **`schedule` 触发器** | 每天晚上要不要跑一次流水线、生成候选文章 | 开着——每晚 21:13（+ 23:37 备份）照常生成候选，落 `posts/` |
| **`publish` 默认值 / `PUBLISH` 环境变量** | 生成的候选校验过了要不要直接发布（跳过草稿位上线） | 关着——自动发布已关闭，默认一律落 `draft: true` 等人工放行 |

大多数时候只需要关心第二层：**候选照常每晚生成，上不上线由你在 `posts/`
里删不删 `draft: true` 那一行决定**（见「[校验没过怎么办](#校验没过怎么办)」）。

#### 完全关掉定时任务（连候选都不生成）

不想让流水线每晚跑、连草稿都不要自动生成，两种做法：

- **推荐，纯 UI 操作，随时可逆**：仓库 → **Actions** → 左侧选 **publish**
  → 右上角 **···** → **Disable workflow**。定时触发和手动触发都会停；
  想恢复就在同一个位置点 **Enable workflow**，不用改代码、不用开 PR
- **改代码**：把 `.github/workflows/publish.yml` 里 `on.schedule` 下两条
  `cron` 整段删掉或注释掉，提交。`push` 到 `drafts/**` 和手动
  `workflow_dispatch` 两条触发方式不受影响，照样能用

#### 重新打开「校验过就直接发布」（不建议，除非清楚后果）

现在没有运行时开关能让**定时任务**直接发布——`publish` 这个
`workflow_dispatch` input 只有手动触发时才存在，定时触发拿不到它，
永远走「不勾」这条路径。想让定时任务恢复成「校验过就自动上线」（改动
之前的行为），要改代码：把 `.github/workflows/publish.yml` 里这一行

```yaml
PUBLISH: ${{ github.event.inputs.publish == 'true' && '--publish' || '' }}
```
...
提交后下一次定时任务、`drafts/` 推送就会在校验通过时直接发布，不再等
人工放行。**改之前想清楚这条线为什么被拉下来**：定时任务曾经默认直接
发布，AI 生成的文章、`tools-` 前缀的工具笔记因此校验一过就绕开人工过目
直接上线，出过两次实测事故（[2026-09-06](docs/devlog/2026-09-06-关闭自动发布.md)、
[2026-09-07](docs/devlog/2026-09-07-关闭自动发布-补工具栏目.md)），改回去等于
重新打开同一个口子。

只想**偶尔**手动跑一次直接发布，不用改代码——手动触发时勾上 `publish`
这个开关就行，见下面「手动触发一次发布」，用完这一次，下次触发照样恢复
默认不勾。

### 手动触发一次发布

Actions → publish → Run workflow，五个开关：
...
`mode: seed` 不填 `seed_url` 时，流水线在体量 2k~40k 字符区间内、还没当过
引子的笔记里按体量从大到小自动选，选中哪篇不受人控制。想自己指定：

1. 去 `Obsidian-base` 仓库里翻到想发的笔记，复制它的 GitHub 链接——地址栏
   那种 `.../blob/master/A/B.md` 形式，中文和斜杠会被转成 `%E6%8A%97`
   这类编码也没关系，粘贴时不用手工处理
2. `seed_url` 填这个链接，`mode` 保持 `seed`，其余照常
3. 那篇笔记不在 `SEED_BAND`（2k~40k 字符）体量区间内也没关系——区间只挡
   自动选材，显式指定的一律放行

链接对不上（拼错、笔记已经被移走）不会误发别的文章：`seed.run()` 找不到
...
了，看 `published.json` **顶部**有没有新增一条没有 `draft` 字段的记录
（字段含义见「[字段说明](#字段说明)」），或者等 `deploy` 跟着跑完后直接
上站看。
